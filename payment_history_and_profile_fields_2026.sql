...
-- Anyone (service role) can insert/update
DROP POLICY IF EXISTS "service_insert_payments" ON public.payment_history;
CREATE POLICY "service_insert_payments" ON public.payment_history
  FOR ALL USING (true) WITH CHECK (true);

-- ── 4. Auto-create payment_history record when subscription is upserted ──
-- (We'll handle inserts from the app/edge function, not via trigger for simplicity)

-- ── 5. Ensure subscriptions has created_at ──
ALTER TABLE public.subscriptions
  ADD COLUMN IF NOT EXISTS created_at timestamptz DEFAULT now();

-- Seed existing subscriptions into payment_history (for existing data)
INSERT INTO public.payment_history (student_id, subscription_id, amount_usd, payment_method, status, plan_name, description, created_at)
SELECT 
  s.student_id,
  s.id,
  COALESCE(s.amount_usd, 0),
  COALESCE(s.payment_method, 'unknown'),
  CASE 
    WHEN s.approved_by_admin = true THEN 'approved'
    WHEN s.approved_by_admin = false AND s.status = 'trial' THEN 'pending'
    WHEN s.approved_by_admin = false THEN 'pending'
    ELSE 'approved'
  END,
  s.plan_name,
  'Suscripción inicial',
