...
CREATE POLICY IF NOT EXISTS "Students manage own completions"
  ON public.unit_completions FOR ALL
  USING (auth.uid() = student_id)
  WITH CHECK (auth.uid() = student_id);

CREATE POLICY IF NOT EXISTS "Admin reads all completions"
...
CREATE POLICY IF NOT EXISTS "Public read payment config"
  ON public.payment_config FOR SELECT
  USING (true);

CREATE POLICY IF NOT EXISTS "Admin write payment config"
...
-- ─────────────────────────────────────────────────────────────────
-- 4. Add required_level to courses for sequential unlocking
-- ─────────────────────────────────────────────────────────────────
